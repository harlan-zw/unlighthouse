...
import type { AuditOpts, Auditor, AuditorReport, Page } from '@unlighthouse/contracts/ports'
...
function failingAuditor(failPaths: string[]): Auditor {
  ...
  return {
    ...
    async audit(url: string, page?: Page, opts?: AuditOpts): Promise<AuditorReport> {
      const u = new URL(url)
...
describe('e2e: createUnlighthouseCore against fixture site', () => {
  ...
  it('cancels mid-scan', async () => {
    ...
    let count = 0
    ;(async () => {
      for await (const _e of session.events) {
        count++
        if (count === 2) {
  ...
  it('error route: one fails, others complete', async () => {
    ...
    expect(failed?.payload.url).toContain('/error')
  }, 10_000)
})
