...
import { roundRobinPick, routeAuditors, splitCategoriesAuditor } from '../packages/core/src/auditors/route'
  ...
  const auditor: Auditor = {
    ...
    async audit(_url: string, _page?: unknown, _opts?: AuditOpts) {
      ...
      for (const c of categories)
        cats[c] = { score: 0.9 }
      return { lighthouseVersion: '13.4.0', categories: cats, audits: {} } as never
    },
  }
  return { name, auditor }
}
...
describe('D-041 splitCategoriesAuditor', () => {
  it('merges disjoint categories with per-category provenance', async () => {
    ...
    const report = await split.audit('https://x.com/', undefined, {}) as unknown as {
  ...
  it('collapses row auditor to the single backend when categories do not diverge', async () => {
    ...
    const report = await split.audit('https://x.com/', undefined, {}) as unknown as { auditor: string }
