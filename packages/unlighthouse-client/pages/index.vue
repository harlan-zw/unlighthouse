    ...
    <TabGroup @change="changedTab" vertical>
      <TabList class="p-1 dark:(bg-blue-900/20 border-none) border-2 border-blue-900/30 rounded-xl 2xl:(mt-8 block) flex">
        ...
        >
          ...
          >
            <span>{{ category }}</span>
            <metric-guage v-if="category !== 'Overview' && !Number.isNaN(categoryScores[key - 1])" :score="categoryScores[key - 1]" stripped class="dark:font-bold" :class="selected ? ['dark:bg-teal-900 bg-blue-100 rounded px-2'] : []" />
          </btn-tab>
        </Tab>
      </TabList>
    </TabGroup>
  ...
  <div class="w-full">
    ...
    <div class="2xl:(max-h-[calc(100vh-100px)]) lg:max-h-[calc(100vh-205px)] sm:max-h-[calc(100vh-220px)] max-h-[calc(100vh-250px)] overflow-auto pr-5 mr-4">
      <div v-if="Object.values(searchResults).length === 0" class="px-4 py-3">
        ...
        <p class="mb-2">No results for search "{{ searchText }}"...</p>
        <btn-action @click="searchText = ''" class="dark:(bg-teal-700) bg-blue-100 px-2 text-sm">Reset Search</btn-action>
        </template>
        ...
        <popover-actions position="left">
          <div class="w-300px flex flex-col space-y-2">
            ...
            >
              ...
              <div class="text-left">
                <p class="break-none text-base">Open Lighthouse Report</p>
            ...
            >
              ...
              <div class="text-left">
                <p class="break-none text-base">Rescan Route</p>
                <span class="opacity-70 text-xs">
      ...
      <div class="min-h-screen px-4 text-center">
        ...
        >
          ...
          >
            ...
            <iframe v-if="iframeModelUrl" :src="iframeModelUrl" class="w-full h-700px bg-white"></iframe>
          </div>
        </TransitionChild>
      </div>
    </div>
