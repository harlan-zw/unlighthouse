...
import { AuditorsList, commands, Health, Manifest, Ready } from '@unlighthouse/contracts/commands'
...
export const manifest: Handler<typeof Manifest> = {
  ...
  async run(_input, ctx) {
    ...
    const hooks = Object.entries(HookSchemas).map(([name, payload]) => ({
      name,
      payloadSchema: toJsonSchema(payload),
    }))
...
export const auditorsList: Handler<typeof AuditorsList> = {
  command: AuditorsList,
  async run(_input, ctx) {
    return AuditorsList.output.parse({
