...
import { isRouteCategory, routeIdentityKey, routeMetricValue, routeNumericValue } from './route-values'
...
export function resolveThresholds(thresholds?: CompareThresholds): CompareThresholds {
  ...
  const legacyBestPractices = thresholds?.bestPractices ?? thresholds?.bestpractices
  if (thresholds?.['best-practices'] == null && legacyBestPractices != null)
    resolved['best-practices'] = legacyBestPractices
  return resolved
}
...
export function selectDetailRows(rows: CompareRouteRow[], filter: DetailFilter = {}, sort = 'delta-perf-desc'): CompareRouteRow[] {
  ...
  if (urlFilter)
    filtered = filtered.filter(route => route.url.includes(urlFilter) || route.path.includes(urlFilter))
...
function extractMetrics(route: ScanRoute): DetailMetrics {
  return mapDetailMetrics(key => routeNumericValue(route, key))
}

function computeDeltas(base: DetailMetrics | null, current: DetailMetrics | null): DetailMetrics {
  return mapDetailMetrics((key) => {
    const baseValue = base?.[key]
    const currentValue = current?.[key]
    return baseValue != null && currentValue != null ? currentValue - baseValue : null
  })
}

function mapDetailMetrics(valueFor: (key: DetailMetricKey) => number | null): DetailMetrics {
  return {
    scorePerformance: valueFor('scorePerformance'),
    scoreAccessibility: valueFor('scoreAccessibility'),
    scoreSeo: valueFor('scoreSeo'),
    scoreBestPractices: valueFor('scoreBestPractices'),
    scoreAgenticBrowsing: valueFor('scoreAgenticBrowsing'),
    lcp: valueFor('lcp'),
    cls: valueFor('cls'),
    inp: valueFor('inp'),
    fcp: valueFor('fcp'),
