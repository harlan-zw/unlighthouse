...
describe('d-029 matrix fan-out', () => {
  it('scans one URL on both devices → two rows, distinct scores per device', async () => {
    ...
    expect(scan?.summary?.routes).toBe(2)
    expect(scan?.summary?.completed).toBe(2)
    expect(scan?.summary?.scoresByDevice?.mobile?.scoresByCategory.performance).toBe(0.9)
    expect(scan?.summary?.scoresByDevice?.desktop?.scoresByCategory.performance).toBe(0.98)
  })
