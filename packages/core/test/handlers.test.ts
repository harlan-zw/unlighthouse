...
describe('handlers — scan.start / scan.rescanAll / history.rescan', () => {
  it('scan.delete rejects the active scan without removing its history', async () => {
    const handlers = createHandlers()
    const ctx = makeCtx()
    await seed(ctx)
    ctx.core = makeCore({ session: () => stubSession() })

    await expect(handlers['scan.delete'].run({ scanId: SCAN_ID }, ctx))
      .rejects
      .toMatchObject({ code: 'ACTIVE_SCAN_CONFLICT' })
    expect(await ctx.storage.scans.get(SCAN_ID)).not.toBeNull()
  })
  ...
  it('scan.rescanAll preserves its source and reuses mode + device matrix', async () => {
    const handlers = createHandlers()
    const ctx = makeCtx()
    await seed(ctx)
    ...
    expect(runCalled).toBe(1)
    expect(receivedOpts?.overrides).toMatchObject({
      site: makeScan().site,
      mode: 'page',
  ...
  it('history.rescan calls core.run() and returns sourceScanId', async () => {
    ...
    ctx.core = makeCore({
      ...
      run: (opts) => {
        runCalled++
        receivedOpts = opts
        return stubSession({ scanId: CLONED_SCAN_ID })
      },
    })
