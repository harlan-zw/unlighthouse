...
import { assertLighthouseResult, extractRouteData, reconcileToContract } from '@unlighthouse/core/report'
...
describe('extract.ts against a Lighthouse 13 LHR', () => {
  it('accepts valid Lighthouse detail payloads outside the projected subset', () => {
    const lhr = lh13Lhr({
      ...
      audits: {
        ...
        'diagnostic-detail-shapes': {
          ...
          details: {
            type: 'list',
            items: [null, 'plain diagnostic text', { node: null, subItems: null }],
          },
        },
      },
    })

    expect(assertLighthouseResult(lhr)).toBe(lhr)
  })
