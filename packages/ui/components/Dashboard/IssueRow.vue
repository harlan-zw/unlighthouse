...
<template>
  <div class="border-b border-default last:border-0">
    <button
    ...
    >
      <div class="flex items-center gap-3 min-w-0 flex-1">
        ...
        <div class="min-w-0 flex-1">
          <div class="text-sm text-highlighted truncate">{{ title }}</div>
          <div v-if="description" class="text-xs text-dimmed truncate mt-0.5">{{ description }}</div>
        </div>
      </div>
      <div class="flex items-center gap-3 shrink-0">
        <SeverityBadge v-if="severity" :severity="severity" />
        <span v-if="count" class="text-xs text-muted font-mono">{{ count }} pages</span>
      </div>
    </button>
