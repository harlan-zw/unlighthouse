...
<template>
  <div class="min-h-screen bg-default text-highlighted flex flex-col items-center justify-center p-6">
    <div class="max-w-md text-center">
      ...
      <details v-if="error.stack" class="mt-8 text-left">
        <summary class="text-sm text-muted cursor-pointer hover:text-highlighted">
          Technical details
        </summary>
        <pre class="mt-2 p-4 bg-muted rounded-lg text-xs text-muted overflow-auto max-h-48 font-mono">{{ error.stack }}</pre>
      </details>
    </div>
  </div>
</template>
