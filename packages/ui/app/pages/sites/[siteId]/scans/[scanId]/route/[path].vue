...
<template>
  <div class="space-y-6">
    <div class="flex items-center gap-3">
      ...
      <UButton color="neutral" variant="ghost" size="sm" @click="backToRoutes">
    ...
    <template v-else>
      ...
      <div class="flex items-start justify-between gap-4">
        <div class="min-w-0">
          ...
          <div class="flex items-center gap-2 mt-1 text-sm text-muted">
            <UBadge variant="outline" class="text-xs">{{ routeData.route?.device }}</UBadge>
        ...
        <div class="flex items-center gap-2">
          ...
          <UButton color="neutral" variant="outline" size="sm" :disabled="rescanning" @click="rescanRoute">
      ...
      <UCard v-if="screenshotVisible">
        <template #header>
          <div class="flex flex-row items-center justify-between">
            <div class="text-sm font-medium text-muted">Visual</div>
            <a
        ...
        <img
          ...
          loading="lazy"
          alt="Page screenshot"
          class="w-full max-w-3xl max-h-[600px] object-contain object-top rounded border border-default bg-muted mx-auto"
      ...
      <div class="grid grid-cols-2" :class="scores.length >= 5 ? 'lg:grid-cols-5' : 'lg:grid-cols-4'" style="gap: 1rem;">
        <UCard v-for="s in scores" :key="s.id">
      ...
      <UCard>
        <template #header>
          <div class="text-sm font-medium text-muted">Core Web Vitals & Metrics</div>
        </template>
        ...
        <UCard>
          <template #header>
            <div class="flex items-center justify-between">
              ...
              <div class="flex items-center gap-2">
                <UBadge v-if="cat.failing.length" color="error" class="text-xs">
                  {{ cat.failing.length }} failing
                </UBadge>
                <UBadge v-if="cat.passing.length" variant="outline" class="text-xs text-green-600">
          ...
          <div class="space-y-4">
            <!-- Failing audits -->
            <div v-if="cat.failing.length" class="w-full divide-y divide-default border-y border-default">
              <details v-for="audit in cat.failing" :key="audit.id" class="group">
                <summary class="flex items-center gap-2 text-sm cursor-pointer list-none py-3">
                  <Icon name="lucide:chevron-right" class="size-4 text-muted shrink-0 transition-transform group-open:rotate-90" />
                  <div class="flex items-center gap-2 text-left flex-1 min-w-0">
                    <UBadge
                      :color="severityColor(audit.severity) === 'destructive' ? 'error' : 'neutral'"
                      :variant="severityColor(audit.severity) === 'destructive' ? 'solid' : severityColor(audit.severity) === 'secondary' ? 'subtle' : 'outline'"
                      class="text-[10px] w-10 justify-center shrink-0"
                    >
                      {{ audit.severity }}
                    </UBadge>
                ...
                <div class="space-y-3 pt-2 pb-3 pl-6">
                  ...
                  <div v-if="audit.metricSavings && hasNonZeroSavings(audit.metricSavings)" class="flex gap-2 flex-wrap">
                    <template v-for="(val, key) in audit.metricSavings" :key="key">
                      <UBadge v-if="typeof val === 'number' ? val > 0 : !!val" variant="outline" class="text-[10px]">
            ...
            <UCollapsible v-if="cat.passing.length">
              <button type="button" class="flex items-center gap-2 w-full text-sm py-1 group">
                <Icon name="lucide:chevron-right" class="size-4 text-muted transition-transform group-data-[state=open]:rotate-90" />
                <Icon name="lucide:check-circle" class="size-4 text-green-500" />
                <span class="text-green-600 font-medium">Passed Audits</span>
                <UBadge variant="outline" class="text-[10px] text-green-600">{{ cat.passing.length }}</UBadge>
              </button>
              <template #content>
                <div class="w-full mt-2 divide-y divide-default border-y border-default">
                  <details v-for="audit in cat.passing" :key="audit.id" class="group">
                    <summary class="flex items-center gap-2 text-sm py-2 cursor-pointer list-none">
                      <Icon name="lucide:chevron-right" class="size-4 text-muted shrink-0 transition-transform group-open:rotate-90" />
                      <div class="flex items-center gap-2 text-left flex-1 min-w-0">
                    ...
                    <div class="space-y-2 pt-1 pb-2 pl-6">
                      <p v-if="audit.description" class="text-xs text-muted" v-html="renderMarkdownLinks(audit.description)" />
                      <div v-if="audit.items?.filter(hasVisibleContent).length" class="border border-default rounded-lg overflow-hidden">
            ...
            <UCollapsible v-if="cat.notApplicable.length">
              <button type="button" class="flex items-center gap-2 w-full text-sm py-1 group">
                <Icon name="lucide:chevron-right" class="size-4 text-muted transition-transform group-data-[state=open]:rotate-90" />
                <Icon name="lucide:minus-circle" class="size-4 text-muted" />
                <span class="text-muted">Not Applicable</span>
                <UBadge variant="outline" class="text-[10px]">{{ cat.notApplicable.length }}</UBadge>
              </button>
              <template #content>
      ...
      <!-- Stack Packs -->
      <UCard v-if="routeData.stackPacks?.length">
        <template #header>
          <div class="text-sm font-medium text-muted">Framework Recommendations</div>
        </template>
      ...
      <!-- Entities -->
      <UCard v-if="routeData.entities?.length">
        <template #header>
          <div class="text-sm font-medium text-muted">Third-Party Entities</div>
        </template>
        <div class="flex flex-wrap gap-2">
          <UBadge
            v-for="entity in routeData.entities"
            :key="entity.name"
            :color="entity.isFirstParty ? 'primary' : 'neutral'"
            :variant="entity.isFirstParty ? 'solid' : 'outline'"
            class="text-xs"
          >
            {{ entity.name }}
          </UBadge>
