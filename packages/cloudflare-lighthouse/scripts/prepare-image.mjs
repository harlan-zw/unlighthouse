// Prepares this package's directory for Docker build by vendoring workspace
// dependencies into ./vendor and emitting a stripped-down package.runtime.json
// that doesn't reference workspace:* protocol.
//
// Run as part of `pnpm build` so `wrangler deploy` (which uses the package
// directory as the Docker build context) sees a self-contained tree.
//
