...
- **Alias:** `it`
  ...
  ```ts
  import { test, expect } from 'unlighthouse'
...
- **Alias:** `it.skip`
  ...
  ```ts
  import { test, assert } from 'unlighthouse'
...
- **Alias:** `it.concurrent`
  ...
  ```ts
  import { describe, test } from 'unlighthouse'
...
When you use `test` in the top level of file, they are collected as part of the implicit suite for it. Using `describe` you can define a new suite in the current context, as a set of related tests and other nested suites. A suite lets you organize your tests so reports are more clear.
  ...
  ```ts
  import { describe, test, expect } from 'unlighthouse'
...
- **Type:** `ExpectStatic & (actual: any) => Assertions`
  ...
  `expect` is used to create assertions. In this context `assertions` are functions that can be called to assert a statement. Unlighthouse provides `chai` assertions by default and also `Jest` compatible assertions build on top of `chai`.
  ...
  ```ts
  import { expect } from 'unlighthouse'
  ...
  Technically this example doesn't use [`test`](#test) function, so in the console you will see Nodejs error instead of Unlighthouse output. To learn more about `test`, please read [next chapter](#test).

  Also, `expect` can be used statically to access matchers functions, described later, and more.

  :::warning
  To provide compatibility with [asymmetric matchers](#expectany), Unlighthouse wraps `.eql` and `.equals` chai assertions, so if you need to use chai equality, you can use `.chaiEqual` matcher.
  :::
...
- **Type:** `beforeEach(fn: () => Awaitable<void>, timeout?: number)`
  ...
  If the function returns a promise, Unlighthouse waits until the promise resolve before running the test.

  Optionally, you can pass a timeout (in milliseconds) defining how long to wait before terminating. The default is 5 seconds.

  ```ts
  import { beforeEach } from 'unlighthouse'
...
- **Type:** `afterEach(fn: () => Awaitable<void>, timeout?: number)`
  ...
  If the function returns a promise, Unlighthouse waits until the promise resolve before continuing.

  Optionally, you can a timeout (in milliseconds) for specifying how long to wait before terminating. The default is 5 seconds.

  ```ts
  import { afterEach } from 'unlighthouse'
...
- **Type:** `beforeAll(fn: () => Awaitable<void>, timeout?: number)`
  ...
  If the function returns a promise, Unlighthouse waits until the promise resolve before running tests.
  ...
  ```ts
  import { beforeAll } from 'unlighthouse'
...
- **Type:** `afterAll(fn: () => Awaitable<void>, timeout?: number)`
  ...
  ```ts
  import { afterAll } from 'unlighthouse'
