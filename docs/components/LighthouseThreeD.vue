...
const showLighthouse = computed(() => {
  return true // scanMeta?.value?.monitor?.status === 'working' || isHovered.value
})
...
<template>
  <div ref="lighthouseRef" :class="showLighthouse ? ['translate-y-3'] : ['opacity-0']" class="transform h-[300px] hover:scale-110 bg-gradient-to-b from-sky-800/90 to-purple-900 dark:bg-none rounded-full duration-2000 ease-in-out transition w-[300px]">
