...
Use the Container-backed auditor for Lighthouse 13 `agentic-browsing` scans. The direct Cloudflare Browser Rendering adapter can run accessibility, SEO, and best-practices audits, but the Worker binding does not expose Chrome feature flags, so it cannot guarantee WebMCP-dependent agentic audits. Even with the Container, WebMCP coverage depends on Cloudflare Browser Run exposing a Chrome/CDP version compatible with Lighthouse's experimental WebMCP checks.
