...
Automate performance monitoring in CI pipelines. Set budgets, generate reports, and catch regressions before production.

## Why Unlighthouse for Lighthouse CI?

Traditional Lighthouse CI scans one page at a time. Unlighthouse CI scans your entire site automatically.

| Feature | Google Lighthouse CI | Unlighthouse CI |
|---------|---------------------|-----------------|
| Pages per run | 1 (or manual URL list) | Entire site |
| URL discovery | Manual configuration | Automatic (sitemap + crawl) |
| Setup complexity | High (lhci wizard, config) | Low (single command) |
| Assertions | Per-page budgets | Site-wide budgets |
| LHCI server | Native | Compatible |
| Static reports | Yes | Yes |

## LHCI Server Integration

Upload results to your existing [LHCI server](https://github.com/GoogleChrome/lighthouse-ci/blob/main/docs/server.md):

```bash
unlighthouse-ci --site example.com --reporter lighthouseServer --lhci-host https://your-lhci-server.com --lhci-build-token <token>
```

Already using LHCI? Unlighthouse extends it with site-wide scanning while keeping your existing server infrastructure.
