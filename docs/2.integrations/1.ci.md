...
## Pushing CI Scans to a Self-Hosted Unlighthouse Dashboard

If you've deployed your own Unlighthouse instance (see
[Self-Host on Cloudflare](/self-host-cloudflare)) you can audit in CI and
POST the result so the dashboard surfaces every run under one site group.

The endpoint is `POST /api/scan/import`. Body is a thin envelope around
the persisted atoms — the same shape `scan.results` / `history.list`
return:

```json
{
  "scan": {
    "scanId": "ci-2026-05-27-abc123",
    "siteId": null,
    ...
    "completedAt": "2026-05-27T15:00:30.000Z",
    "ciBranch": "main",
    "ciCommit": "abc123",
    "ciCommitMessage": "ship the perf fix",
    "summary": {
      "routes": 12, "completed": 12, "failed": 0,
      "scoreAverage": 0.93,
      "scoresByCategory": { "performance": 0.91, "accessibility": 0.92, "best-practices": 0.95, "seo": 0.98, "agentic-browsing": 0.60 },
      "durationMs": 30000
    }
  },
  "routes": [
    {
      ...
      "path": "/",
      "scorePerformance": 0.91,
      "lcp": 2100, "cls": 0.02, "inp": 150,
      "...": "rest of ExtractedMetrics fields"
    }
  ],
  "packRuns": []
}
...
Curl example (CI step that ships a local scan up to the dashboard):

```bash
unlighthouse-ci --site example.com --reporter json > report.json
curl -X POST https://unlighthouse.yourcompany.com/api/scan/import \
...
```json
{ "scanId": "ci-2026-05-27-abc123", "imported": true, "routes": 12, "packRuns": 0 }
```

The imported scan appears immediately in `/history` and `/sites`,
grouped by the `site` URL alongside any other scans for that host.

### Replacing an existing scan

`scan.import` is **no-overwrite by design** — re-POSTing a `scanId` that
already exists returns `SCAN_ALREADY_EXISTS`. Delete first if you intend
to replace:

```bash
curl -X POST $HOST/api/scan/delete -d '{"scanId":"ci-2026-05-27-abc123"}'
curl -X POST $HOST/api/scan/import -d @report.json
```

### Multi-device matrix

Each row in `routes[]` carries its own `device` field. Push a
`mobile + desktop` matrix in one request by including rows for both
devices; the dashboard's `/history` view pairs them onto the same line
automatically (5-min start-time window).

### Auth

There is **no auth gate** in v1 — the endpoint assumes the host is bound
to localhost or a private network. For multi-tenant hosted deploys, wrap
the storage adapter with `wrapStorage({ hooks })` and add a bearer-token
check in the `before` hook of `scans.create` / `routes.putBatch`.

### Hidden from MCP

`scan.import` is **server-side ingestion only** — it is not exposed to
MCP agents. Agents that need to seed scans should use `scan.start`
instead, which goes through the normal audit pipeline.
  ...
  demo:
    ...
    steps:
      ...
      - name: Install Dependencies
        run: npm install -g unlighthouse puppeteer netlify-cli
